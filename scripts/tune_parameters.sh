...
#1st arg: path to file to be modified, 2nd arg: index of parameter to modify (3,4 or 6), 3rd arg: optional arguments to pass
crash_modify() {
    cd ${resultdir}
...
handle_gpu_warning() {
    ...
    if [ "${comparison}" = '>2.0' ]; then
        ...
        #already modified ultra_thresh so try K or B
        if [ ${modified_K} -le ${modified_B} ]; then
            #modify K
...
param_test() {
    ...
    if [ -z "${3}" ]; then
        command="scripts/param_test.sh -D ${1} -n ${2} ${bam_file_arg} ${read_file_arg} ${genome_file_arg}"
    else
...
first_run() {
    ...
    if [ ${skip_warnings} -eq 0 ]; then
        #if we want to heed warnings, then do it after the first run
        ...
        #if we don't want to heed warnings, then simply create a .profile file after the first run
        python3 process_results create_profile "${dataset}/${run_number}/useful_${run_number}.txt"
    fi
    cd ${exedir}
}
...
help_msg() {
    ...
    echo "-b [bam file]        Path to the .bam file to be used."
    echo "-r [read file]       Path to the .fastq file to be used."
    echo "-g [ref genome]      Path to the reference genome to be used."
    ...
    echo "-s                   Flag to indicate that only the binary search should be performed. Requires previous results to exist."
...
# parse options
while getopts t:s:d:b:r:g:n:B:K:swhc opt
do
	case $opt in
		t) testdir="$OPTARG";;
        s) resultdir="$OPTARG";;
		d) dataset="$OPTARG";;
        b) bam_file_arg="-b $OPTARG";;
        r) read_file_arg="-r $OPTARG";;
        g) genome_file_arg="-g $OPTARG";;
        n) num_runs=$((${OPTARG}));;
        s) binary_only=1;;
        w) skip_warnings=1;;
        B) max_bases=$((${OPTARG}));;
...
#optimize parameters that affect crashes first
optimize_indices=(6)
...
for i in ${optimize_indices[@]}; do
    ...
    cat "${dataset}/best.profile" > "${dataset}/best_${i}.profile"
